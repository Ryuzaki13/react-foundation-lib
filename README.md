...
| `configureRowBasedFormatterRegistry`                   | Устанавливает созданный host-приложением реестр до компиляции pipeline.                                 |
| `getRowBasedFormatterList`, `getRowBasedFormatterById` | Читает активный реестр host-приложения.                                                                 |

Пакет не содержит прикладных row-based формул. Host-приложение владеет definitions, создаёт реестр через factory и устанавливает его в composition root до первого вызова pipeline API.
...
Тесты: [`registry.test.ts`](./src/formulas/registry.test.ts), [`execute.test.ts`](./src/formulas/execute.test.ts), [`validate.test.ts`](./src/formulas/validate.test.ts), [`perf.runtime.test.ts`](./src/formulas/perf.runtime.test.ts).
