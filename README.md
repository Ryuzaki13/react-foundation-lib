...
Импортируй публичные API точечно из subpath exports пакета: `@ryuzaki13/react-foundation-lib/formatters`, `@ryuzaki13/react-foundation-lib/odata-service`, `@ryuzaki13/react-foundation-lib/query-client`. Корневой импорт `@ryuzaki13/react-foundation-lib` намеренно не является публичным API. Ссылки ниже ведут в исходники и тесты, чтобы можно было быстро проверить поведение.
...
4. Не относится ли логика к пакетам `@ryuzaki13/react-foundation-api`, `@ryuzaki13/react-foundation-ui`, доменному слою или конкретному приложению.

Если функция нужна только одному приложению или slice, не тащи ее в этот пакет. Если функция общая, имя должно быть точным: не `utils`, не `prepare`, не `process`.
