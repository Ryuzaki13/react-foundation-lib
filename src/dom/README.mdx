...
5. вызывает `.focus({ preventScroll: true })`, только если target — подключённый `HTMLElement`;
...
через `.focus({ preventScroll: true })`. Открытие и закрытие overlay не меняют
прокрутку документа, в том числе пока портал ещё позиционируется. Переходы
по Tab сохраняют обычную прокрутку к выбранному элементу внутри длинного overlay.
