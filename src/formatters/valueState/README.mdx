...
<Meta title="Foundation Lib/API/Formatters/Value State" />
...
Pipeline регистрирует resolver на этапе компиляции, вычисляет state до typed formatting и возвращает state/icon-параметры UI. Подробности: [pipeline/README.mdx](../pipeline/README.mdx#шаг-resolvevaluestate).
