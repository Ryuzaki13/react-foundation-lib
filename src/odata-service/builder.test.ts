...
describe("odata-service builder", () => {
	...
	it("собирает entity path и operation path", () => {
		...
		expect(buildEntityPath(entityMetadata, config, params)).toBe("/ZDEMO_SRV/DemoSet(p_code='AB',p_flag='X')/Results");
		expect(buildEntityOperationPath(entityMetadata, config, params, "query")).toBe(
			"/ZDEMO_SRV/DemoSet(p_code='AB',p_flag='X')/Results"
		);
		expect(buildEntityOperationPath(entityMetadata, config, params, "create")).toBe("/ZDEMO_SRV/DemoSet");
		expect(buildEntityOperationPath(entityMetadata, config, params, "update")).toBe("/ZDEMO_SRV/DemoSet(p_code='AB',p_flag='X')");
	});
	...
	it("игнорирует query params для непараметризованного EntitySet и предупреждает в dev", () => {
		...
		expect(buildEntityOperationPath(metadata, config, { p_code: { value: "AB" } }, "query")).toBe("/ZDEMO_SRV/DemoSet");
	...
	it("собирает FunctionImport query params без encodeURIComponent в values API", () => {
		...
		expect(buildFunctionImportPath(functionImportMetadata, config, { p_code: { value: "AB" } })).toBe(
			"/ZDEMO_SRV/DemoSet?p_code=%27AB%27"
		);
	});

	it("возвращает base path для FunctionImport без параметров", () => {
		expect(buildFunctionImportPath({ ...functionImportMetadata, parameters: undefined }, config, {})).toBe("/ZDEMO_SRV/DemoSet");
	});
});
