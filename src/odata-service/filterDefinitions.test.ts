...
describe("sanitize filter definitions", () => {
	...
	it("нормализует definitions и выводит физические columnIds из binding", () => {
		const definitions: ODataCompiledFilterDefinition[] = [
			{
				...
				controlType: "string",
				dictionaryCodeKey: " ZSEG "
			},
			{
				id: " TREE ",
				ownerColumnId: " UNKNOWN ",
				columnIds: [" ZCFO ", "ZCFO", " ZDEP "],
				kind: "tree",
			...
			{ id: " ADV ", ownerColumnId: " ZADV ", kind: "advanced", componentId: "advanced-search-select", configId: "cfg" },
			{ id: " COL ", ownerColumnId: " ZCOL ", kind: "column" }
		];
		...
		expect(sanitizeFilterDefinitions(definitions)).toMatchObject([
			...
			{ id: "ADV", ownerColumnId: "ZADV" },
			{ id: "COL", ownerColumnId: "ZCOL" }
		]);
	});
	...
	it("санитизирует значения фильтров, dedupe и patch-обновления", () => {
		...
		expect(sanitizeFilterValue({ " ZDIV ": [" 01 ", "01", ""], " ": ["x"], ZBAD: [1, ""] })).toEqual({
			ZDIV: ["01"]
		});
		...
		expect(normalizeDictionaryCodeKey(" ZCODE ", "ZOWNER")).toBe("ZCODE");
		expect(normalizeDictionaryCodeKey("ZOWNER", "ZOWNER")).toBeUndefined();
	});
});
...
describe("compileFiltersToExpression", () => {
	...
	it("возвращает активные колонки для segment, column, advanced, boolean и value-фильтров", () => {
		...
		expect(resolveODataFilterDefinitionActiveColumnIds(definitions[0]!, "A")).toEqual(["ZSEG"]);
	...
	it("возвращает активные колонки tree-фильтра по выбранным значениям", () => {
		...
		expect(resolveODataFilterDefinitionActiveColumnIds(definition, { ZCFO: ["01"] })).toEqual(["ZCFO"]);
	...
	it("компилирует segment по owner-колонке, а зависимости отдаёт по кодовому полю справочника", () => {
		const definitions: ODataCompiledFilterDefinition[] = [
			{
				id: "ZCFO_ZDEP",
				ownerColumnId: "ZCFO_ZDEP",
				columnIds: ["ZCFO_ZDEP"],
				kind: "segment",
				componentId: "multi-select",
				controlType: "string",
				dictionaryCodeKey: "ZCFO1_ZDEP"
			}
		];
		const values = {
			ZCFO_ZDEP: ["001", "002"]
	...
	it("компилирует local value без explicit conditions через default eq", () => {
		...
		expect(buildODataFilter(compileFiltersToExpression(definitions, { VALUE: ["B", "A"] }))).toBe("(STATUS eq 'A' or STATUS eq 'B')");
		expect(compileFiltersToExpression(definitions, { VALUE: { STATUS: ["A"] } })).toBeUndefined();
	});
	...
	it("flatten dependencies поддерживает scalar segment, tree и пропускает local", () => {
		...
		expect(flattenFilterValuesToODataDependencies(definitions, { SEG: "A", TREE: { ZCFO1: ["01"] }, LOCAL: true })).toEqual({
			ZSEG: ["A"],
			ZCFO1: ["01"]
		});
	...
	it("оставляет только самый глубокий segment-фильтр в OData-цепочке", () => {
		expect(
			collapseChainedODataSegmentFilterValues(
				...
				{
					main: [
						{ codeKey: "ZDIV", count: 10 },
						{ codeKey: "ZCFO1", count: 40 }
					]
				}
			)
		).toEqual({
	...
	it("collapse chained segment values не трогает пустые values и не-OData segment filters", () => {
		expect(collapseChainedODataSegmentFilterValues({}, ["ZDIV"], {})).toEqual({});
		expect(
			collapseChainedODataSegmentFilterValues(
				{
					ZDIV: ["01"],
