...
describe("image api xml", () => {
	it("собирает компактный SOAP XML без переносов строк", () => {
		const xml = buildSoapEnvelope("TEST_FN", `<DATA>${buildXmlFields({ TEXT_VALUE: "123" })}</DATA>`);

		expect(xml).toContain("<urn:TEST_FN><DATA><TEXT_VALUE>123</TEXT_VALUE></DATA></urn:TEST_FN>");
		expect(xml).not.toContain("\n");
	});

	it("экранирует специальные символы в значениях", () => {
		const xml = buildXmlFields({ TEXT_VALUE: `a&b<c>d"e'f` });

		expect(xml).toBe("<TEXT_VALUE>a&amp;b&lt;c&gt;d&quot;e&apos;f</TEXT_VALUE>");
	});
